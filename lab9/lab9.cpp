...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
    ...
    bool quad = (mesh == meshes["square"]);
    glUniform1i(glGetUniformLocation(shader->program, "quad"), quad);
    glUniform1i(glGetUniformLocation(shader->program, "mixedT"), mixT);
...
Texture2D* Lab9::CreateRandomTexture(unsigned int width, unsigned int height)
{
    ...
    glGenTextures(1, &randomT);
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, randomT);
